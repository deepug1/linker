  ...
  if ($$(".form-submit-button")) {
    $$(".form-submit-button").each(function (b) {
      ...
      if (b.className != "form-submit-button lastDisabled") {
        if (
          b.classList.contains("js-new-sacl-button") &&
...
qq.enableSubmitButton = function () {
  var liSize =
    $$("ul.qq-upload-list li").size() -
    $$("ul.qq-upload-list li.qq-upload-fail").size();
...
qq.FileUploaderBasic.prototype = {
          ...
          setTimeout(function () {
            window.getFilesOfInput(input).then(function (files) {
              files.file.forEach(function (file) {
                var previewItem = document.querySelector(
                  'li[actual-filename="' + file.fileName + '"]'
                );
                if (previewItem) {
  ...
  _isRealImage: function (input) {
    ...
    var realFileCheck = function (input, index) {
      ...
      var isCheckPossible =
        ...
        typeof window.ImageInfo != "undefined";
  ...
  _isFileuploadv2: function () {
    var uploadBtn = this._listElement
      .up(".form-line")
      .querySelector(".jfUpload-button");
  ...
  _validateFile: function (file) {
    ...
    if (!this._isAllowedExtension(name)) {
      ...
      size &&
      this._options.sizeLimitActive !== "No" &&
  ...
  _checkFileLimit: function () {
    ...
    var isThereMoreFileThanLimit =
      fileUploaderExists && validFiles > this._options.fileLimit;
  ...
  _errorAccesibility: function (fileName, message) {
    var deleteElement = this._element.select(
      '[actual-filename="' + fileName + '"] .qq-upload-delete'
    );
...
qq.getUploaderTemplate = function (buttonText, fieldName = "") {
  ...
  return (
    ...
    '<div class="qq-upload-button {buttonStyle}" aria-hidden="true">' +
    buttontext +
    "</div>" +
    '<div class="inputContainer"></div>' +
    '<label class="form-sub-label" aria-hidden="true" for="input_' +
    fieldID +
    '" id="sublabel_' +
    fieldID +
    '">{subLabel}</label>' +
...
qq.FileUploader = function (o) {
  ...
  qq.extend(this._options, {
    ...
    subLabel: "",
    buttonText: "",
    template: qq.getUploaderTemplate(
      this._options.buttonText,
      this._options.params.field
    ),
  ...
  this._element.innerHTML = this._options.template
    .replace("{subLabel}", this._options.subLabel)
    .replace("{of}", this._options.ofText)
    .replace("{cancel}", this._options.cancelText);

  if (this._options.buttonStyle) {
    this._element.innerHTML = this._element.innerHTML.replace(
      "{buttonStyle}",
...
qq.extend(qq.FileUploader.prototype, {
  ...
  _onProgress: function (id, fileName, loaded, total) {
    ...
    try {
      ...
      var text;
      if (loaded != total) {
        var ofText = this._listElement
          .up(".form-line")
          .down(".ofText").innerText;
        text =
  ...
  _onComplete: function (id, fileName, result, file) {
    ...
    typeof qq.onUploadComplete === "function" &&
      qq.onUploadComplete(item, id, fileName, result, qid);
    //Emre
    ...
    if (this._isFileuploadv2() && uploadedFile) {
      ...
      fileReader.onloadend = function () {
        ...
        var fileExt = uploadedFile.name.substring(
  ...
  _addToList: function (id, fileName) {
    var self = this;
    var cancel = this._listElement
      .up(".form-line")
      .down(".cancelText").innerText;
    var item = qq.toElement(
      this._options.fileTemplate.replace(/\{Cancel\}/, cancel)
    );
    ...
    qq.setText(
      fileElement,
      this._formatFileName(fileName, this._listElement.clientWidth)
    );
  ...
  _deleteFromList: function (item, fileName) {
    ...
    if (item.readAttribute("actual-filename"))
      fileName = item.readAttribute("actual-filename");
        ...
        onComplete: function (t) {
          ...
          if (t.success) {
            JotForm.removeFile({
              fileElement: item,
              fileName: fileName,
              hiddenTempUploadInput: $(
                [opts.params.folder, opts.params.field, fileName].join("_")
              ),
            });

            typeof opts.onDelete === "function" &&
              opts.onDelete(opts.params.folder, opts.params.field, fileName);
            typeof qq.onDelete === "function" &&
              qq.onDelete(qid, opts.params.folder, opts.params.field, fileName);
          } else {
            JotForm.errored(
              item,
              t.error || "Something went wrong! Please try again later."
            );
          }
...
qq.UploadDropZone.prototype = {
  ...
  _isValidFileDrag: function (e) {
    ...
    // Sentry #1542177212
    var isIE11atWin81 =
      navigator.userAgent.indexOf("NT 6.3; WOW64; Trident/7.0") > -1;
    var isIE11atWin7 =
      navigator.userAgent.indexOf("NT 6.1; WOW64; Trident/7.0") > -1;
...
qq.UploadButton.prototype = {
  ...
  _createInput: function () {
    ...
    if (window.FORM_MODE === "cardform") {
      var uploadButtonView = this._element
        .up(".jfCard")
        .down(".jfUpload-button");
    ...
    var failedListElement = this._element
      .up()
      .down("ul.qq-upload-list li.qq-upload-fail");
    if (failedListElement) {
      ...
      if (deleteElement) {
        ...
        inputContainer.setAttribute(
          "aria-label",
          defaultAriaLabel + " " + errorText.replace("So, delete it.", "")
        );
      }
    }
    ...
    function alignButtonsAndAddListener(cb) {
      ...
      if (window.FORM_MODE === "cardform") {
        if (
          document.querySelector(".isVisible .qq-upload-button") ===
          self._element
        ) {
          uploadButtonEl = document.querySelector(
            ".isVisible .jfUpload-button"
          );
        }
      } else {
  ...
  _onComplete: function (id, xhr) {
    ...
    if (xhr.status == 200) {
      ...
      this._options.onComplete(
        id,
        name,
        { error: JotForm.texts.multipleFileUploads_uploadFailed },
        this._xhrs[id].file
      );
    }
